...
.controller('PanelCtrl',['$scope', '$location', '$filter', '$timeout','$rootScope', 'panel', 'Search','Filter', 'ENV', 'Restangular','localStorageService','Navigation','_','Authentication', function ($scope, $location, $filter, $timeout, $rootScope, panel, Search, Filter, ENV, Restangular,localStorageService,Navigation,_,Authentication) {
	...
	$scope.panel.nextPreviousFig = {'previous':(figIdx===0)?null:figIdx-1,'next':(figIdx===$scope.panel.paper.figures.length-1)?null:figIdx+1};
	...
	$scope.getSimilarCollections = function(searchScope){
		...
		$scope.similarCollections = [];
		$scope.carouselIndex = 0;
		var visiblePanels = $filter('similarSearchFilter')($scope.similarPanels,{similarSearchScope:searchScope,paper_id: panel.paper.paper_id});
	...
	Restangular.one('panel',panel.current_panel_id).get({get_similar: true}).then(function(data){
		...
		$scope.similarSearchScope = ($filter('similarSearchFilter')($scope.similarPanels,{similarSearchScope:'global',paper_id: panel.paper.paper_id}).length) ? "global" : "paper";
	...
	if(Navigation.graph.nodes.length){ $scope.graph = Navigation.graph ;}
	else{
...
//==================== SUMMARY VIEW ===============================//
	...
	var hasExtIdANDsameExternalId = function(tag1, tag2, sameStatus){
		...
		var filterExtId1 =_.filter(tag1.external_id,function(tag1Ext){ return tag1Ext.status !='rejected' && tag1Ext.status !='deleted';});
		var filterExtId2 =_.filter(tag2.external_id,function(tag2Ext){ return tag2Ext.status !='rejected' && tag2Ext.status !='deleted';});

		if(!filterExtId1.length || !filterExtId2.length) return false;

		var extIdTag1 = _.map(filterExtId1,function(tag1Ext){ return tag1Ext.database_id +'-'+ tag1Ext.id + ((sameStatus)?('-' + tag1Ext.status):'');});
		var extIdTag2 = _.map(filterExtId2,function(tag2Ext){ return tag2Ext.database_id +'-'+ tag2Ext.id + ((sameStatus)?('-' + tag2Ext.status):'');});
				...
				angular.forEach(condensedTags, function(cTag,cTagIdx){
					...
					if(c4SameStatus && c3SameTandR && (c1||c2)){
						...
						if(sameTags[cTag.id]){
							...
							sameTags[cTag.id] = {'nb':2, 'ids':[cTag.paper_tag_id,tag.paper_tag_id], 'panels':panels};
						}
						var newIsDefault = false;
						angular.forEach(tag.external_id,function(tagExtId){ if(tagExtId.status == 'default') newIsDefault = true; });
						if(newIsDefault){
	...
	var getSummaryView = function(currentTags){
		var summaryView = {component:[],experiment:[],intervention:[],reporter:[],normalizing:[],assayed:[],assay:[], trash:[]};
		...
		summaryView.assay = _.uniq(summaryView.assay,function(tag){return tag.tooltipContent+tag.text;});
		summaryView.component = _.uniq(summaryView.component,function(tag){return tag.tooltipContent+tag.text;});
		summaryView.experiment = _.uniq(summaryView.experiment,function(tag){return tag.tooltipContent+tag.text;});
		summaryView.intervention = _.uniq(summaryView.intervention,function(tag){return tag.tooltipContent+tag.text;});
		summaryView.reporter = _.uniq(summaryView.reporter,function(tag){return tag.tooltipContent+tag.text;});
		summaryView.normalizing = _.uniq(summaryView.normalizing,function(tag){return tag.tooltipContent+tag.text;});
		summaryView.assayed = _.uniq(summaryView.assayed,function(tag){return tag.tooltipContent+tag.text;});
		summaryView.trash = _.uniq(summaryView.trash,function(tag){return tag.tooltipContent+tag.text;});
		return summaryView;
